...
def get_blog_list_common_data(request,blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.EACH_PAGE_BLOGS_NUMBER)  # 没10个进行分页
    ...
    context['blog_types'] = BlogType.objects.annotate(blog_count=Count('blog')) #获取分类下贴子的数量
...
def blog_detail(request,blog_id):
    ...
    read_cooklie_key=read_statistics_once_read(request,blog)
    context['blog']=blog
    ...
    response.set_cookie(read_cooklie_key,'true')#阅读cookie标记 
    return response
